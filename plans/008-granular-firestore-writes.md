...
> in `plans/README.md`.
>
> **Drift check (run first)**: `git diff --stat 0bcd99f..HEAD -- src/services/firestoreService.js src/pages/BubblesPage.js src/hooks/useMatterEngine.js`
...
1. **Создание** — `src/pages/BubblesPage.js:882-886`:
   ```js
   setBubbles(prev => {
   ...
   ```
2. **Сохранение правки** — `BubblesPage.js:941-982` (внутри `setBubbles(prev => prev.map(...))`, side effect на строке 980). Обновлённый объект строится из `selectedBubble` + form-state (`title`, `description`, `selectedTagId`, `editBubbleSize`, `newBody`, `editDueDate`, `editNotifications`, `editRecurrence`, перерасчёт overdue-полей).
3. **Удаление тега из задач** — `BubblesPage.js:1231-1243` (внутри `setBubbles`, по таймеру 7 с; обнуляет `tagId` у затронутых).
4. **Переключение rich-text** — `BubblesPage.js:1921-1925` (внутри `setBubbles`).
5. **Импорт JSON** — `BubblesPage.js:1979` (`await saveBubblesToFirestore(importedBubbles)`) — **легитимная** массовая операция, остаётся.
6. **Остановка пульсации** — `BubblesPage.js:2623-2627` (внутри `setBubbles`; меняет `overduePulseSuppressed/overdueSticky/overdueAt/updatedAt` одной задачи).
...
- `src/pages/BubblesPage.js` (только перечисленные call-site'ы)
...
`BubblesPage.js:882-886` переписать — side effect наружу, точечная запись:
...
В обработчике сохранения (район `BubblesPage.js:915-985`): обновлённый объект задачи сегодня строится внутри `prev.map(...)`. Вынести построение наружу: собрать `updatedBubble` из `selectedBubble` и form-state теми же выражениями (строки 944-976 — перенести вычисления `newDueDate`, `shouldDisablePulsing`, `dateChanged` и сам объект до setState), затем:
...
- `handleToggleEditUseRichText` (`BubblesPage.js:1916-1926`):
  ```js
  ...
  ```
- Остановка пульсации (`BubblesPage.js:2614-2627`): объект `updatedBubble` уже строится до setState — оставить, но вместо `saveBubblesToFirestore(updated)` внутри апдейтера:
  ```js
  setBubbles(prev => prev.map(b => b.id === selectedBubble.id ? updatedBubble : b));
...
**Verify**: `grep -n 'saveBubblesToFirestore' src/pages/BubblesPage.js` → остались только call-site 3 (deleteTag) и 5 (import).

### Step 5: Удаление тега из задач (call-site 3)

В таймере `handleDeleteTag` (`BubblesPage.js:1231-1243`): side effect наружу, точечные записи по затронутым задачам:
...
**Verify**: build зелёный; `grep -n 'saveBubblesToFirestore' src/pages/BubblesPage.js` → ровно 1 вхождение (import, строка ~1979).
...
- [ ] `grep -rn 'saveBubblesToFirestore' src/` → ровно 3 вхождения: определение в `firestoreService.js`, вызов миграции в `loadBubblesFromFirestore`, вызов импорта в `BubblesPage.js`
- [ ] `grep -c 'upsertBubble\|updateBubbleFields\|deleteBubbleDoc' src/services/firestoreService.js` → ≥3 (определения)
- [ ] Внутри всех `setBubbles(prev => ...)` в `BubblesPage.js` нет вызовов функций сервиса: `grep -n 'saveBubblesToFirestore\|upsertBubble\|updateBubbleFields' src/pages/BubblesPage.js` — ни одно вхождение не находится внутри колбэка `setBubbles` (проверить глазами все совпадения)
